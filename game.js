...
Game.prototype.render = function() {
 ...
 // Inside the render function where you draw the winText
 if (this.winText) {
     ctx.fillStyle = this.winTextColor;
     ctx.font = "60px arial";
     ctx.fillText(this.winText, 400, 300);
     ctx.font = "20px arial";
     ctx.fillText('Tap to continue...', 400, 350);
     ctx.font = "30px arial";
 }
...
// Add this code after the Game constructor

// Add the following code after the Game constructor
...
// Add the following functions after the previous code
...
Game.prototype.handleMouseMove = function (x, y) {
    ...
    for (vKey in vertices) {
        v = vertices[vKey];
        if (this.eqTolerance(v.px, x) && this.eqTolerance(v.py, y) && v.active === 0) {
    ...
    if (x > 600) {
        this.alphaSpeed = +(x - 600) / 6000;
    } else if (x < 200) {
        this.alphaSpeed = +(x - 200) / 6000;
    } else {
        this.alphaSpeed = 0.0005;
    }

    if (y > 520) {
        this.betaSpeed = -0.009;
    } else if (y < 100) {
        this.betaSpeed = +0.009;
    } else {
        this.betaSpeed = 0.000;
    }
};

Game.prototype.handleMouseUp = function (x, y) {
    var vertices = this.vertices,
        v, vKey;
    ...
    for (vKey in vertices) {
        ...
        if (this.eqTolerance(v.px, x) && this.eqTolerance(v.py, y) && v.active === 0) {
            ...
            this.doMove(this.player, v);
            this.winCheck(); // (needed if solid vertices count is odd)

            this.aiMoveTimer = window.setTimeout(function () {

                this.player *= -1;
                this.aiMove(this.player, this.level - 1);
                this.winCheck();
                this.player *= -1;
            }.bind(this), 350);
...
// Add these functions at the end of your existing code
